@section('side-navbar')
    <div class="sidebar" data-background-color="white" data-active-color="warning">
      ...
      <img src="{{ asset('images/logo.png') }}" alt="Sofitel Philippine Plaza Hotel" style="width:50px; height:50px;">
                ...
                @if($admin_nav)
                    @foreach($admin_nav as $item)
                        ...
                        <li @if (Request::is('admin/'.strtolower($item['name']).'/create*') || Request::is('admin/'.strtolower($item['name']))) class="active" @endif>
                            <a data-toggle="collapse" href="#componentstest{{ $loop->index }}">
                                <i class="{{$item['icon']}}"></i>
                                <p>{{$item['name']}}
                                    <b class="caret"></b>
                                </p>
                            </a>
                            <div @if (Request::is('admin/'.strtolower($item['name']).'/create*') || Request::is('admin/'.strtolower($item['name']))) class="collapse in" @else class="collapse" @endif id="componentstest{{ $loop->index }}">
                                <ul class="nav">
